...
const AppSidebar: React.FC = () => {
  ...
  const { hasPermission } = useAuth(); // 👈 Obtener función hasPermission
  ...
  // 👇 Función para verificar si un item debe mostrarse según permisos
  const shouldShowItem = (item: NavItem): boolean => {
    if (!item.permission) return true; // Si no requiere permiso, mostrar siempre
    return hasPermission(item.permission); // Verificar si tiene el permiso
  };
  ...
  const renderMenuItems = (items: NavItem[], menuType: "main" | "others") => (
      <ul className="flex flex-col gap-4">
        {items
            .filter(shouldShowItem) // 👈 Filtrar items según permisos
            .map((nav, index) => (
                <li key={nav.name}>
                  {nav.subItems ? (
                      <button
